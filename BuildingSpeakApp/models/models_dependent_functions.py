...
#################
def temp_func(account):
    this_month = pd.Period(timezone.now(),freq='M')
    this_month = this_month - 20
    last_month = this_month - 1   #link user selection here
    
    bldg_data = []
    for bldg in account.building_set.order_by('name'):
        bldg_view_data_this = bldg.get_building_view_data(month_first=this_month,
                                                          month_last=this_month)
        if bldg_view_data_this is None: bldg_view_data_this = [False, False]
        bldg_view_data_last = bldg.get_building_view_data(month_first=last_month,
                                                          month_last=last_month)
        if bldg_view_data_last is None: bldg_view_data_last = [False, False]
        bldg_data.append([bldg,
                          bldg_view_data_this[0],
                          bldg_view_data_this[1],
                          bldg_view_data_last[0],
                          bldg_view_data_last[1],
                          ])
    return bldg_data
