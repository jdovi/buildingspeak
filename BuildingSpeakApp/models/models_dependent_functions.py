...
#################
def temp_func(account, month_first=pd.Period(timezone.now(),freq='M')-40, month_last=pd.Period(timezone.now(),freq='M')):
    ...
    if len(account.meter_set.all()) < 1:
        result = None
    else:
        ...
        utility_groups.extend(sorted(set([str(x.utility_type) for x in account.meter_set.all()])))
        ...
        meter_dict = {'Total Building Energy': {'name': 'Total Building Energy',
                                                ...
                                                'df': convert_units_sum_meters(
                                                        'other', 
                                                        'kBtuh,kBtu', 
                                                        account.meter_set.filter(~Q(utility_type = 'domestic water')), 
                                                        first_month=first_month, 
                                                        last_month=last_month )
                                                } }
        
        #cycle through all utility types present in this account, get info and dataframes
        for utype in sorted(set([str(x.utility_type) for x in account.meter_set.all()])):
            utype = str(utype)
            meter_dict[utype] = {}
            ...
            meter_dict[utype]['df'] = convert_units_sum_meters(
                                        ...
                                        account.meter_set.filter(utility_type=utype),
                                        first_month=first_month, 
                                        last_month=last_month)
        ...
        for utype in meter_dict.keys():
            ...
            bill_data[cost_per_sf] = bill_data['Cost (act)'] / account.building_set.all().aggregate(Sum('square_footage'))['square_footage__sum']
            bill_data[consumption] = bill_data['Consumption (act)']
            bill_data[consumption_per_day] = bill_data['Consumption (act)'] / bill_data['Days']
            bill_data[consumption_per_sf] = bill_data['Consumption (act)'] / account.building_set.all().aggregate(Sum('square_footage'))['square_footage__sum']
            ...
            for m in bill_data_totals.index:
                ...
                if i in [j.month for j in bill_data.index]:
                    ...
                    bill_data_totals[cost_per_sf][m] = bill_data['Cost (act)'][[x.month==i for x in bill_data.index]].sum() / account.building_set.all().aggregate(Sum('square_footage'))['square_footage__sum']
                    ...
                    bill_data_totals[consumption_per_sf][m] = bill_data['Consumption (act)'][[x.month==i for x in bill_data.index]].sum() / account.building_set.all().aggregate(Sum('square_footage'))['square_footage__sum']
            ...
            bill_data_totals[cost_per_sf]['Annual'] =           bill_data['Cost (act)'].sum() / account.building_set.all().aggregate(Sum('square_footage'))['square_footage__sum']
            ...
            bill_data_totals[consumption_per_sf]['Annual'] =    bill_data['Consumption (act)'].sum() / account.building_set.all().aggregate(Sum('square_footage'))['square_footage__sum']
        ...
        for meter in account.meter_set.all():
